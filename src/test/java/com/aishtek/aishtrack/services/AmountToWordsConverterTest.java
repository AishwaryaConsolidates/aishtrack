...
public class AmountToWordsConverterTest extends BaseIntegrationTest {
  ...
  public void itConvertsBigDecimalAmountToWords() throws IOException {
    ...
    assertEquals(AmountToWordsConverter.convert(amount), "Rupees One and Twelve Paisa only");

    amount = new BigDecimal("12.12");
    assertEquals(AmountToWordsConverter.convert(amount), "Rupees Twelve and Twelve Paisa only");

    amount = new BigDecimal("123.12");
    assertEquals(AmountToWordsConverter.convert(amount),
