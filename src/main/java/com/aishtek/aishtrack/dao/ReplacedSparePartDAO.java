...
public class ReplacedSparePartDAO extends BaseDAO {

  public static int create(Connection connection, SparePart sparePart) throws SQLException {
    PreparedStatement preparedStatement = connection.prepareStatement(
        "insert into visit_replaced_spare_parts (visit_id, part_number, description, quantity) values(?, ?, ?, ?)",
    ...
    preparedStatement.setInt(4, sparePart.getQuantity());
      ...
      throw new SQLException("ReplacedSparePart not generted");
      }
  }

  public static void update(Connection connection, SparePart sparePart) throws SQLException {
    PreparedStatement preparedStatement = connection.prepareStatement(
        "update visit_replaced_spare_parts set part_number = ?, description =?, quantity =? where id = ?");
  ...
  public static ArrayList<SparePart> findByVisitId(Connection connection, int visitId)
      throws SQLException {
    String sql =
        "SELECT id, visit_id, part_number, description, quantity FROM visit_replaced_spare_parts where visit_id = ?";
